...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            #img_list = [im for _ in range(time_step)]
            #print(im[7][0][2])
            #im = im.repeat(time_step, 1, 1, 1, 1)
            preds, train_out = model(im) if compute_loss else (model(im, augment=augment), None)
            #print(preds[0][0][0])
            #print(train_out[0][0][0])
            #im = im[0]
        # Loss
        if compute_loss:
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/urpc.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'checkpoint.pt', help='model path(s)')
