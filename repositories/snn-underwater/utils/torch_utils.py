...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
    ...
    for v in model.modules():
        ...
        if isinstance(v, bn):  # weight (no decay)
         ...
         # 添加手动定义的 nn.Parameter
        for param_name, param in v.named_parameters(recurse=False):
            if 'p' in param_name and isinstance(param, nn.Parameter):  # Example for parameter 'p'
                g[1].append(param)  # Add to the no decay group, or adjust according to your needs
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        # Create EMA
        #print(model)
        self.ema = de_parallel(model).eval()  # FP32 EMA
